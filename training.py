...
parser.add_option("-j",
                  ...
                  dest="inDirVal",
                  default='./',
                  help="input directory for validation data")
...
if (options.contigLength is None or options.filter_len1 is None
        or options.nb_filter1 is None or options.nb_dense is None):
...
# training
filename_codetrfw = [
    x for x in os.listdir(inDirTr)
    if 'codefw.npy' in x and 'virus' in x and contigLengthk + 'k' in x
][0]
print("data for training " + filename_codetrfw)
phageRef_codetrfw = np.load(os.path.join(inDirTr, filename_codetrfw))
phageRef_codetrbw = np.load(
    os.path.join(inDirTr, filename_codetrfw.replace('fw', 'bw')))
# validation
filename_codevalfw = [
    x for x in os.listdir(inDirVal)
    if 'codefw.npy' in x and 'virus' in x and contigLengthk + 'k' in x
][0]
print("data for validation " + filename_codevalfw)
phageRef_codevalfw = np.load(os.path.join(inDirVal, filename_codevalfw))
phageRef_codevalbw = np.load(
    os.path.join(inDirVal, filename_codevalfw.replace('fw', 'bw')))
...
filename_codetrfw = [
    x for x in os.listdir(inDirTr)
    if 'codefw.npy' in x and 'host' in x and contigLengthk + 'k' in x
][0]
print("data for training " + filename_codetrfw)
hostRef_codetrfw = np.load(os.path.join(inDirTr, filename_codetrfw))
hostRef_codetrbw = np.load(
    os.path.join(inDirTr, filename_codetrfw.replace('fw', 'bw')))
# validation
filename_codevalfw = [
    x for x in os.listdir(inDirVal)
...
hostRef_codevalbw = np.load(
    os.path.join(inDirVal, filename_codevalfw.replace('fw', 'bw')))

######## combine V and H, shuf training data ##########
print("...combining V and H...")
### training V+B
Y_tr = np.concatenate((np.repeat(0, hostRef_codetrfw.shape[0]),
                       np.repeat(1, phageRef_codetrfw.shape[0])))
...
X_trfw_shuf = X_trfw[np.ix_(index_trfw, range(X_trfw.shape[1]),
                            range(X_trfw.shape[2]))]
del X_trfw
X_trbw_shuf = X_trbw[np.ix_(index_trfw, range(X_trbw.shape[1]),
                            range(X_trbw.shape[2]))]
del X_trbw
Y_tr_shuf = Y_tr[index_trfw]

### validation V+B
Y_val = np.concatenate((np.repeat(0, hostRef_codevalfw.shape[0]),
                        np.repeat(1, phageRef_codevalfw.shape[0])))
...
batch_size = int(
    X_trfw_shuf.shape[0] /
    (1000 * 1000 / contigLength))  ## smaller batch size can reduce memory
pool_len1 = int((contigLength - filter_len1 + 1) / POOL_FACTOR)

modPattern = 'model_siamese_varlen_' + contigLengthk + 'k_fl' + str(
    filter_len1) + '_fn' + str(nb_filter1) + '_dn' + str(nb_dense)
#modName = os.path.join( outDir, modPattern +'_ep{epoch:02d}_acc{acc:.2f}'+'.h5')
modName = os.path.join(outDir, modPattern + '.h5')
checkpointer = keras.callbacks.ModelCheckpoint(filepath=modName,
                                               verbose=1,
                                               save_best_only=True)
...
auc = sklearn.metrics.roc_auc_score(Y, Y_pred)
print('auc_' + type + '=' + str(auc) + '\n')
np.savetxt(os.path.join(outDir, modPattern + '_' + type + 'fw_Y_pred.txt'),
           np.transpose(Y_pred))
np.savetxt(os.path.join(outDir, modPattern + '_' + type + 'fw_Y_true.txt'),
           np.transpose(Y))
